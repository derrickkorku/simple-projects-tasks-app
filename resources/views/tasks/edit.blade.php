        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="p-6 mt-6 bg-white border-b border-gray-200 overflow-hidden shadow-sm sm:rounded-lg">
                <form action="{{ route('tasks.update', $task)  }}" method="post">
                    ...
                    <x-text-input :value="$task->name" class="w-full" field="name" type="text" name="name" id="name" placeholder="{{ __('Enter task name') }}" autocomplete="off" />

                    <label for="priority" class="mt-2">{{ __('Priority') }}</label>
                    <x-text-input :value="$task->priority" class="w-full" field="priority" type="number" name="priority" id="priority" placeholder="{{ __('Enter task priority number') }}" autocomplete="off" />
                    ...
                    <x-primary-button class="mt-3">{{ __('Update Task') }}</x-primary-button>
                </form>
            </div>
        </div>
    </div>
